...
Naršyklė ir OANDA collector neturi jungtis tiesiai prie DB. Ryšio schema:

```text
Naršyklė / OANDA collector -> HTTPS/WSS -> Goldie API -> TLS -> PostgreSQL
```
...
- Prisijungus iš kito tinklo galima autentifikuotis Goldie UI ir matyti tuos
  pačius duomenis.
- OANDA collector gali siųsti duomenis į vidinį HTTPS API.
