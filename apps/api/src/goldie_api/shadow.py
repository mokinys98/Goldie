...
def create_signal_outcome(db: Session, signal: Signal, tick: MarketTick) -> SignalOutcome | None:
    ...
    db.scalar(select(Bot).where(Bot.id == signal.bot_id).with_for_update())
    base = {
        "signal_id": signal.id,
    ...
    account = db.scalar(
        ...
        .where(AccountSnapshot.bot_id == signal.bot_id)
    ...
    if account is None:
        outcome = SignalOutcome(
            **base,
            status="SKIPPED",
            skip_reason="MISSING_ACCOUNT_DATA",
        )
        db.add(outcome)
        return outcome
    ...
    spec = db.scalar(
        ...
        .where(
            SymbolSpecification.bot_id == signal.bot_id,
            SymbolSpecification.symbol == tick.symbol,
        )
...
def evaluate_open_outcome(db: Session, tick: MarketTick) -> SignalOutcome | None:
    outcome = db.scalar(
        select(SignalOutcome).where(
            SignalOutcome.bot_id == tick.bot_id,
            SignalOutcome.status == "OPEN",
        )
    )
    ...
    spec = db.scalar(
        select(SymbolSpecification)
        .where(
            SymbolSpecification.bot_id == tick.bot_id,
            SymbolSpecification.symbol == tick.symbol,
        )
        .order_by(desc(SymbolSpecification.updated_at))
    )
